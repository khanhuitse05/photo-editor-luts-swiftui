                ...
                VStack(alignment: .leading, spacing: 8) {
                    HStack {
                        ...
                        Button("Close", action: {
                            ...
                            .accessibilityHint("Closes the road map view")
                    }
                    Text("Product\nRoad map")
                        .font(.largeTitle)
                        .fontWeight(.semibold)
                    ...
                    Text(AppContent.roadMapDescription)
                        .font(.body)
                        .foregroundStyle(.secondary)
                        .padding(.vertical)
                    ...
                    Group {
                        ForEach(AppContent.roadMapContent) { item in
                            RoadStepView(
                                title: item.title,
                                content: item.body,
                                date: item.date,
...
struct RoadStepView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel("\(title). \(content). \(date)")
    }
}
